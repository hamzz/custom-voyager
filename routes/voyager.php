...
use JMI\Voyager\Events\Routing;
use JMI\Voyager\Events\RoutingAdmin;
use JMI\Voyager\Events\RoutingAdminAfter;
use JMI\Voyager\Events\RoutingAfter;
...
Route::group(['as' => 'voyager.'], function () {
    ...
    if (config('cas.enable') == true) {
        $middleware = ['cas.user'];
    } else {
        $middleware = [];
    }

    Route::get('login', ['uses' => $namespacePrefix.'VoyagerAuthController@login',     'as' => 'login'])->middleware($middleware);

    Route::post('login', ['uses' => $namespacePrefix.'VoyagerAuthController@postLogin', 'as' => 'postlogin']);

    Route::group(['middleware' => array_merge(['admin.user'], $middleware)], function () use ($namespacePrefix) {
